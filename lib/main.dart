...
const _privacyUrl = 'https://graviitysmassh.com/privacy-policy.html';
const _supportUrl = 'https://graviitysmassh.com/support.html';

void main() {
  WidgetsFlutterBinding.ensureInitialized();
  runApp(const GravitySmashApp());
}

class GravitySmashApp extends StatelessWidget {
  const GravitySmashApp({super.key});
...
class _LoadingScreenState extends State<LoadingScreen> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 55), (timer) async {
      if (!mounted) {
        return;
      }
      setState(() => _progress = (_progress + 0.025).clamp(0, 1));
      if (_progress >= 1) {
        timer.cancel();
      ...
      body: Stack(
        ...
        children: [
          ...
          Align(
            ...
            child: _GameTitle(fontSize: isLandscape ? 54 : 58),
          ),
          Align(
            alignment: const Alignment(0, 0.78),
            child: Padding(
              padding: EdgeInsets.symmetric(horizontal: isLandscape ? 260 : 42),
              child: Column(
                ...
                children: [
                  ...
                  _NeonProgressBar(value: _progress),
                ],
              ),
            ),
          ),
  ...
  Future<void> _openWebView(String title, String value) async {
    await Navigator.of(context).push<void>(
      MaterialPageRoute<void>(
        builder: (_) => LegalWebViewScreen(title: title, url: value),
      ),
    );
  }
                      ...
                      children: [
                        Expanded(
                          child: _SmallLinkButton(
                            text: 'Privacy Policy',
                            onPressed: () =>
                                _openWebView('Privacy Policy', _privacyUrl),
                          ),
                        ),
                        ...
                        Expanded(
                          child: _SmallLinkButton(
                            text: 'Support',
                            onPressed: () =>
                                _openWebView('Support', _supportUrl),
                          ),
                        ),
                      ],
                    ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0xAA00D4FF), blurRadius: 18)],
      ),
      child: ClipRRect(
